...
## 文件清单
- `detector.py` — 文件类型检测，双重验证，中文路径兼容（~198行）
- `router.py` — 策略路由，动态加载处理器（~174行）
- `embedder.py` — 统一嵌入接口，含自动验证、回滚和审计（~190行）
- `extractor.py` — 统一提取接口，含审计日志（~115行）
- `verifier.py` — 独立验证接口，支持单文件和批量验证（~110行）
